...
import { authAPI, habboAPI } from '../../services/api';
...
export function AuthForm({ type, onSubmit, onToggle }: AuthFormProps) {
  ...
  const handleStep1Submit = async () => {
    ...
    try {
      ...
      setCurrentStep(2);
      setErrors({});
      toast.success('Kullanıcı bulundu! Şimdi doğrulama kodunu mottona ekle.');
    } catch (error) {
      setErrors({ habboUsername: 'Habbo kullanıcı adı bulunamadı!' });
  ...
  const handleVerification = async () => {
    ...
    try {
      // Habbo kullanıcısının mottosunu çek
      const userProfile = await habboAPI.getUserProfile(formData.habboUsername);
      const userMotto = userProfile.motto || '';
      
      if (userMotto.includes(generatedCode)) {
        ...
        setErrors({ verification: 'Doğrulama kodu mottonda bulunamadı!' });
        toast.error('Doğrulama kodu mottonda bulunamadı!');
      }
    } catch (error) {
      setVerificationStatus('failed');
      setErrors({ verification: 'Doğrulama sırasında hata oluştu!' });
    ...
    if (Object.keys(newErrors).length === 0) {
      ...
      try {
        const user = authAPI.register({
          ...
          email: `${formData.habboUsername}@toh.com`,
      ...
      <div className="space-y-6">
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          ...
          <Button
            ...
            disabled={loading}
            icon={LogIn}
            className="bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 py-4 text-lg"
          >
            Giriş Yap
          </Button>
        </form>
        ...
        {currentStep === 1 && (
          ...
          >
            ...
            <Button
              ...
              loading={loading}
              disabled={loading}
              className="bg-gradient-to-r from-blue-500 to-purple-500 hover:from-blue-600 hover:to-purple-600 py-4 text-lg"
            >
              Devam Et
            </Button>
          </motion.div>
          ...
          >
            ...
            <div className="bg-gradient-to-r from-orange-50 to-red-50 border-2 border-orange-200 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-4">
                <span className="text-sm font-medium text-gray-700">Doğrulama Kodu:</span>
                <Button
                  onClick={copyCode}
                  variant="ghost"
                  size="sm"
              ...
              <div className="bg-white rounded-xl p-4 border-2 border-dashed border-orange-300">
                <code className="text-2xl font-bold text-orange-600 block text-center">
                  {generatedCode}
                </code>
              </div>
            ...
            <Button
              ...
              loading={loading}
              disabled={loading}
              className="bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-600 hover:to-orange-600 py-4 text-lg"
            >
        ...
        {currentStep === 3 && (
          ...
          >
            ...
            <Button
              ...
              disabled={loading}
              icon={UserPlus}
              className="bg-gradient-to-r from-green-500 to-blue-500 hover:from-green-600 hover:to-blue-600 py-4 text-lg"
            >
              TÖH'e Katıl
            </Button>
